      ...
      {carregando ? (
        ...
        <div className="grid grid-cols-2 gap-3">
          {estadias.map((e) => {
            ...
            return (
              <div key={e.id} className="flex flex-col justify-between p-4 rounded-3xl relative"
                style={{ background: '#252015', border: deletando === e.id ? '1.5px solid #7A3A2A' : '1.5px solid #3A2E22', minHeight: 120 }}>
                {deletando === e.id ? (
                  <div className="flex flex-col items-center justify-center h-full gap-3 py-2">
                    <p className="text-xs font-semibold text-center" style={{ color: '#F2EBE0' }}>Apagar planejamento?</p>
                  ...
                  <>
                    <div className="flex items-start justify-between gap-2 mb-3">
                      <Link href={`/estadia/${e.id}`} className="flex-1 min-w-0">
                        <p className="font-semibold text-sm leading-tight truncate" style={{ color: '#F2EBE0' }}>{e.nome}</p>
                      </Link>
                      <div className="flex items-center gap-2 flex-shrink-0">
                        <button onClick={() => duplicar(e.id)}
                          ...
                          <svg width="13" height="13" viewBox="0 0 24 24" fill="none">
                            <rect x="9" y="9" width="13" height="13" rx="2" stroke="#9B8B7A" strokeWidth="2"/>
                            <path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1" stroke="#9B8B7A" strokeWidth="2"/>
                          </svg>
                        </button>
                        <button onClick={() => setDeletando(e.id)}
                          className="text-base leading-none opacity-30 hover:opacity-70"
                          style={{ color: '#9B8B7A' }}>×</button>
                      </div>
                    </div>
                    <Link href={`/estadia/${e.id}`}>
                      <p className="text-xs font-medium mb-1" style={{ color: '#C4823A' }}>{periodo(e)}</p>
          ...
          <Link href="/estadia/nova"
            ...
            style={{ border: '1.5px dashed #3A2E22', background: 'transparent', minHeight: 120 }}>
            <div className="w-8 h-8 rounded-xl flex items-center justify-center mb-2" style={{ background: '#2A2118' }}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none">
                <path d="M12 5v14M5 12h14" stroke="#C4823A" strokeWidth="2.5" strokeLinecap="round"/>
              </svg>
            </div>
            <p className="text-xs font-semibold text-center" style={{ color: '#C4823A' }}>Novo planejamento</p>
          </Link>
        </div>
      )}
    </main>
